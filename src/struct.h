...
#define CH_STRUCT_BODY_IMPL(struct_name, value_name, const_name, assignment_body, field_body, ...) \
  public:\
    ...
    struct_name(const cash::internal::ch_bitbase<struct_name::bitcount>& __rhs__) { this->assign(__rhs__); } \
  ...
  protected: \
    void read_data(cash::internal::nodelist& __inout__, size_t __offset__, size_t __length__) const { \
      CH_FOR_EACH(CH_STRUCT_READ_DATA, CH_SEP_SEMICOLON, __VA_ARGS__); \
    } \
    void write_data(size_t __dst_offset__, const cash::internal::nodelist& __in__, size_t __src_offset__, size_t __length__) { \
...
#define CH_STRUCT_IMPL(struct_name, ...) \
  class struct_name : public cash::internal::ch_bitbase<CH_STRUCT_SIZE(__VA_ARGS__)> { \
  protected: \
