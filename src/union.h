...
#define CH_UNION_BODY_IMPL(union_name, value_name, const_name, assignment_body, field_body, ...) \
  ...
  protected: \
    ...
    void read_data(cash::internal::nodelist& __inout__, size_t __offset__, size_t __length__) const override { \
      cash::internal::read_data(_, __inout__, __offset__, __length__); \
    } \
    void write_data(size_t __dst_offset__, const cash::internal::nodelist& __in__, size_t __src_offset__, size_t __length__) override { \
...
#define CH_UNION_IMPL(union_name, ...) \
  class union_name : public cash::internal::ch_bitbase<CH_UNION_SIZE(__VA_ARGS__)> { \
  protected: \
