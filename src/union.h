...
#define CH_UNION_FIELD(i, x) \
  ch::internal::sliceref<base, CH_PAIR_L(x)::bitcount> CH_PAIR_R(x)

#define CH_UNION_CONST_FIELD(i, x) \
  ch::internal::const_sliceref<ch::internal::refproxy<base>, CH_PAIR_L(x)::bitcount> CH_PAIR_R(x)

#define CH_UNION_COPY_CTOR_APPLY(i, x) \
  CH_PAIR_R(x)(_)

#define CH_UNION_BODY_IMPL(union_name, value_name, const_name, assignment_body, field_body, ...) \
  public: \
    using base = ch::internal::ch_bitbase<CH_UNION_SIZE(__VA_ARGS__)>; \
    using value_type = value_name; \
    using const_type = const_name; \
    union_name() : CH_FOR_EACH(CH_UNION_COPY_CTOR_APPLY, CH_SEP_COMMA, __VA_ARGS__) {} \
    union_name(const union_name& __rhs__) : CH_FOR_EACH(CH_UNION_COPY_CTOR_APPLY, CH_SEP_COMMA, __VA_ARGS__), _(__rhs__._) {} \
    union_name(union_name&& __rhs__) : CH_FOR_EACH(CH_UNION_COPY_CTOR_APPLY, CH_SEP_COMMA, __VA_ARGS__), _(std::move(__rhs__._)) {} \
    union_name(const ch_bitbase<union_name::bitcount>& __rhs__) : CH_FOR_EACH(CH_UNION_COPY_CTOR_APPLY, CH_SEP_COMMA, __VA_ARGS__), _(__rhs__) {} \
    template <typename T, CH_REQUIRES(ch::internal::is_bit_convertible<T, base::bitcount>::value)> \
    explicit union_name(const T& __rhs__) \
      : CH_FOR_EACH(CH_UNION_COPY_CTOR_APPLY, CH_SEP_COMMA, __VA_ARGS__), \
      _(static_cast<typename ch::internal::bitbase_cast<T, base::bitcount>::type>(__rhs__)) {} \
...
#define CH_UNION_IMPL(union_name, ...) \
  class union_name : public ch::internal::ch_bitbase<CH_UNION_SIZE(__VA_ARGS__)> { \
  protected: \
    class __const_type__ : public ch::internal::ch_bitbase<CH_UNION_SIZE(__VA_ARGS__)> { \
      CH_UNION_BODY_IMPL(__const_type__, union_name, __const_type__, CH_UNION_READONLY_IMPL, CH_UNION_CONST_FIELD, __VA_ARGS__) \
    }; \
    CH_UNION_BODY_IMPL(union_name, union_name, __const_type__, CH_UNION_WRITABLE_IMPL, CH_UNION_FIELD, __VA_ARGS__) \
  }
