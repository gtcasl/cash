...
struct union_zero_init_impl1 {
  static void apply(T& obj) {
    if constexpr(is_logic_traits<typename T::traits>::value && bitwidth_v<U> < T::traits::bitwidth) {
      obj.asBits() = 0;
    }
  };
};
...
#define CH_UNION_DEFAULT_CTOR(i, x) \
  CH_PAIR_R(x)(ch::internal::type_buffer_t<traits>(ch_bitwidth_v<ch::internal::identity_t<CH_PAIR_L(x)>>, buffer, 0))
...
#define CH_UNION_BODY_IMPL(union_name, reverse_name, field_body, ...) \
...
#define CH_UNION_WRITABLE_IMPL(union_name, reverse_name, ...) \
  ...
  union_name& operator=(const reverse_name& rhs) { \
    ch::internal::type_accessor_t<traits>::copy(*this, rhs); \
    return *this; \
  }
...
#define CH_UNION_IMPL(union_name, ...) \
  ...
  private: \
    ...
    public: \
      ...
      CH_UNION_BODY_IMPL(__scalar_const_type__, __scalar_type__, CH_UNION_CONST_SCALAR_FIELD, __VA_ARGS__) \
    ...
    public: \
      ...
      CH_UNION_BODY_IMPL(__scalar_type__, __scalar_const_type__, CH_UNION_SCALAR_FIELD, __VA_ARGS__) \
