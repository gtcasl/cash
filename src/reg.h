...
inline void ch_pushClock(const T& clk) {
  static_assert(T::bitsize == 1, "invalid predicate size");
...
template <typename T, typename I,
          ...
          CH_REQUIRES(is_bit_convertible<T, deduce_type_t<T, I>::bitsize>::value),
          CH_REQUIRES(is_bit_convertible<I, deduce_type_t<T, I>::bitsize>::value)>
...
const auto ch_latch(const T& next, const E& enable, const I& init, const Z& reset) {
  static_assert(E::bitsize == 1, "invalid predicate size");
  static_assert(Z::bitsize == 1, "invalid predicate size");
  return make_type<deduce_first_type_t<T, I>>(
