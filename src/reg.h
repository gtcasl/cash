...
inline void ch_pushClock(const bitbase<1>& clk) {
...
inline void ch_pushReset(const bitbase<1>& reset) {
...
template <unsigned N>
const auto ch_ready(const bitbase<N>& x) {
  return make_type<ch_bit<1>>(createReadyNode(get_lnode(x)));
}

template <unsigned N>
const auto ch_valid(const bitbase<N>& x) {
...
template <typename T, typename I,
          ...
          CH_REQUIRES(is_ch_bit_convertible<T, deduce_type_t<T, I>::bitsize>::value),
          CH_REQUIRES(is_ch_bit_convertible<I, deduce_type_t<T, I>::bitsize>::value)>
const auto ch_latch(const T& next, const bitbase<1>& enable, const I& init, const bitbase<1>& reset) {
  return make_type<deduce_first_type_t<T, I>>(
...
template <typename T, typename I,
          ...
          CH_REQUIRES(is_ch_bit_convertible<T, deduce_type_t<T, I>::bitsize>::value),
          CH_REQUIRES(is_ch_bit_convertible<I, deduce_type_t<T, I>::bitsize>::value)>
const auto ch_latch(const T& next, const bitbase<1>& enable, const I& init) {
  return make_type<deduce_first_type_t<T, I>>(
    createLatchNode(get_lnode<T, deduce_type_t<T, I>::bitsize>(next),
                    get_lnode<I, deduce_type_t<T, I>::bitsize>(init),
                    get_lnode(enable),
...
template <typename T,
          CH_REQUIRES(is_ch_bit_convertible<T>::value)>
const auto ch_latch(const T& next, const bitbase<1>& enable) {
  return make_type<T>(
