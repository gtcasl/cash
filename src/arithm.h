...
#define CH_BINOP_GEN(func, op) \
  template <typename A, typename B, \
            typename = typename std::enable_if<(deduce_bitcount<A, B>::value != 0)>::type, \
            typename = typename if_convertible<A, ch_bit<deduce_bitcount<A, B>::value>>::type, \
            typename = typename if_convertible<B, ch_bit<deduce_bitcount<A, B>::value>>::type> \
...
#define CH_SHIFTOP_GEN(func, op) \
  template <typename A, typename B, \
            typename = typename std::enable_if<(max_bitcount<A, B>::value != 0)>::type, \
            typename = typename if_convertible<A, ch_bit<first_bitcount<A, B>::value>>::type, \
            typename = typename if_convertible<B, ch_bit<first_bitcount<B, A>::value>>::type> \
  const auto op(const A& a, const B& b) { \
    return func(a, b); \
  }
...
const auto OpBinary(const A& a, const B& b) {
  return make_bit<N>(createAluNode(op, get_node<A, N>(a), get_node<B, N>(b)));
}

template <ch_alu_op op, unsigned N, typename A, typename B>
const auto OpCompare(const A& a, const B& b) {
  return make_bit<1>(createAluNode(op, get_node<A, N>(a), get_node<B, N>(b)));
}
...
template <typename I, typename S,
          typename = typename if_convertible<I, ch_bit<I::bitcount>>::type,
          typename = typename if_convertible<S, ch_bit<S::bitcount>>::type>
const auto ch_mux(const I& in, const S& sel) {
  return make_bit<(I::bitcount >> S::bitcount)>(
        createAluNode(alu_op_mux, get_node(in), get_node(sel)));
}
