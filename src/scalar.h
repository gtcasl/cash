...
struct scalar_accessor {
  ...
  static void copy(U& dst, const V& src) {
    ...
    d->first.copy(d_offset, s->first, s_offset, U::bitsize);
    ++d->second;
  }

  template <typename T>
  static scalar_buffer clone(const T& obj) {
    scalar_buffer ret(T::bitsize);
    auto& s = obj.get_buffer().get_source();
    auto s_offset = obj.get_buffer().get_offset();
    ret.write(0, s->first.get_words(), s->first.get_cbsize(), s_offset, T::bitsize);
    return ret;
  }
};
...
public:
  ...
  ch_scalar(const ch_scalar& rhs) : buffer_(scalar_accessor::clone(rhs)) {}

  ch_scalar(ch_scalar&& rhs) : buffer_(std::move(rhs.buffer_)) {}

  ch_scalar(const const_scalar_slice<N>& rhs)
    : buffer_(scalar_accessor::clone(rhs)) {}
  ...
  explicit ch_scalar(const T& rhs) : buffer_(scalar_accessor::clone(rhs)) {}
