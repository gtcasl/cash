...
#define CH_LOGIC_INT_OP1(i, x) \
  CH_LOGIC_OP1_IMPL((), <, const ch_int&, x) \
  CH_LOGIC_OP1_IMPL((), <=, const ch_int&, x) \
  CH_LOGIC_OP1_IMPL((), >, const ch_int&, x) \
  CH_LOGIC_OP1_IMPL((), >=, const ch_int&, x)

#define CH_LOGIC_INT_FUNC1(i, x) \
  CH_LOGIC_FUNC1_IMPL((template <unsigned N>), ch_lt, const ch_int<N>&, x) \
  CH_LOGIC_FUNC1_IMPL((template <unsigned N>), ch_le, const ch_int<N>&, x) \
  CH_LOGIC_FUNC1_IMPL((template <unsigned N>), ch_gt, const ch_int<N>&, x) \
  CH_LOGIC_FUNC1_IMPL((template <unsigned N>), ch_ge, const ch_int<N>&, x)

#define CH_LOGIC_INT_OP1_TYPES \
  const ch_scint<N>&, int8_t, int16_t, int32_t, int64_t

#define CH_LOGIC_INT_OP5(i, x) \
  CH_LOGIC_OP2_IMPL((template <unsigned M, CH_REQUIRE_0(M < N)>), <, const ch_int&, x) \
  CH_LOGIC_OP2_IMPL((template <unsigned M, CH_REQUIRE_0(M < N)>), <=, const ch_int&, x) \
  CH_LOGIC_OP2_IMPL((template <unsigned M, CH_REQUIRE_0(M < N)>), >, const ch_int&, x) \
  CH_LOGIC_OP2_IMPL((template <unsigned M, CH_REQUIRE_0(M < N)>), >=, const ch_int&, x)

#define CH_LOGIC_INT_FUNC5(i, x) \
  CH_LOGIC_FUNC2_IMPL((template <unsigned N, unsigned M, CH_REQUIRE_0(M < N)>), ch_lt, const ch_int<N>&, x) \
  CH_LOGIC_FUNC2_IMPL((template <unsigned N, unsigned M, CH_REQUIRE_0(M < N)>), ch_le, const ch_int<N>&, x) \
  CH_LOGIC_FUNC2_IMPL((template <unsigned N, unsigned M, CH_REQUIRE_0(M < N)>), ch_gt, const ch_int<N>&, x) \
  CH_LOGIC_FUNC2_IMPL((template <unsigned N, unsigned M, CH_REQUIRE_0(M < N)>), ch_ge, const ch_int<N>&, x)
...
#define CH_LOGIC_INT_FUNC2(i, x) \
  CH_LOGIC_FUNC1_IMPL((template <unsigned N>), ch_and, const ch_int<N>&, x) \
  CH_LOGIC_FUNC1_IMPL((template <unsigned N>), ch_or, const ch_int<N>&, x) \
  CH_LOGIC_FUNC1_IMPL((template <unsigned N>), ch_xor, const ch_int<N>&, x) \
  CH_LOGIC_FUNC1_IMPL((template <unsigned N>), ch_add, const ch_int<N>&, x) \
  CH_LOGIC_FUNC1_IMPL((template <unsigned N>), ch_sub, const ch_int<N>&, x) \
  CH_LOGIC_FUNC1_IMPL((template <unsigned N>), ch_mult, const ch_int<N>&, x) \
  CH_LOGIC_FUNC1_IMPL((template <unsigned N>), ch_div, const ch_int<N>&, x) \
  CH_LOGIC_FUNC1_IMPL((template <unsigned N>), ch_mod, const ch_int<N>&, x)
...
#define CH_LOGIC_INT_FUNC6(i, x) \
  CH_LOGIC_FUNC2_IMPL((template <unsigned N, unsigned M, CH_REQUIRE_0(M < N)>), ch_and, const ch_int<N>&, x) \
  CH_LOGIC_FUNC2_IMPL((template <unsigned N, unsigned M, CH_REQUIRE_0(M < N)>), ch_or, const ch_int<N>&, x) \
  CH_LOGIC_FUNC2_IMPL((template <unsigned N, unsigned M, CH_REQUIRE_0(M < N)>), ch_xor, const ch_int<N>&, x) \
  CH_LOGIC_FUNC2_IMPL((template <unsigned N, unsigned M, CH_REQUIRE_0(M < N)>), ch_add, const ch_int<N>&, x) \
  CH_LOGIC_FUNC2_IMPL((template <unsigned N, unsigned M, CH_REQUIRE_0(M < N)>), ch_sub, const ch_int<N>&, x) \
  CH_LOGIC_FUNC2_IMPL((template <unsigned N, unsigned M, CH_REQUIRE_0(M < N)>), ch_mult, const ch_int<N>&, x) \
  CH_LOGIC_FUNC2_IMPL((template <unsigned N, unsigned M, CH_REQUIRE_0(M < N)>), ch_div, const ch_int<N>&, x) \
  CH_LOGIC_FUNC2_IMPL((template <unsigned N, unsigned M, CH_REQUIRE_0(M < N)>), ch_mod, const ch_int<N>&, x)
...
#define CH_LOGIC_INT_OP3(i, x) \
  CH_LOGIC_OP3_IMPL((), <<, const ch_int&, x, const ch_int<CH_WIDTH_OF(x)>&) \
  CH_LOGIC_OP3_IMPL((), >>, const ch_int&, x, const ch_int<CH_WIDTH_OF(x)>&)

#define CH_LOGIC_INT_FUNC3(i, x) \
  CH_LOGIC_FUNC3_IMPL((template <unsigned N>), ch_sll, const ch_int<N>&, x, const ch_int<CH_WIDTH_OF(x)>&) \
  CH_LOGIC_FUNC3_IMPL((template <unsigned N>), ch_srl, const ch_int<N>&, x, const ch_int<CH_WIDTH_OF(x)>&)

#define CH_LOGIC_INT_OP3_TYPES \
  int8_t, uint8_t, int16_t, uint16_t, int32_t, uint32_t, int64_t, uint64_t

#define CH_LOGIC_INT_OP7(i, x) \
  CH_LOGIC_OP3_IMPL((template <unsigned M>), <<, const ch_int&, x, const ch_int<M>&) \
  CH_LOGIC_OP3_IMPL((template <unsigned M>), >>, const ch_int&, x, const ch_int<M>&)

#define CH_LOGIC_INT_FUNC7(i, x) \
  CH_LOGIC_FUNC3_IMPL((template <unsigned N, unsigned M>), ch_sll, const ch_int<N>&, x, const ch_int<M>&) \
  CH_LOGIC_FUNC3_IMPL((template <unsigned N, unsigned M>), ch_srl, const ch_int<N>&, x, const ch_int<M>&)

#define CH_LOGIC_INT_OP7_TYPES \
  const ch_scalar<M>&

#define CH_LOGIC_INT_OP4(i, x) \
  CH_LOGIC_OP4_IMPL((), <<, const ch_int<CH_WIDTH_OF(x)>&, x, const ch_int&) \
  CH_LOGIC_OP4_IMPL((), >>, const ch_int<CH_WIDTH_OF(x)>&, x, const ch_int&)

#define CH_LOGIC_INT_FUNC4(i, x) \
  CH_LOGIC_FUNC4_IMPL((template <unsigned N>), ch_sll, const ch_int<CH_WIDTH_OF(x)>&, x, const ch_int<N>&) \
  CH_LOGIC_FUNC4_IMPL((template <unsigned N>), ch_srl, const ch_int<CH_WIDTH_OF(x)>&, x, const ch_int<N>&)

#define CH_LOGIC_INT_OP4_TYPES \
  int8_t, uint8_t, int16_t, uint16_t, int32_t, uint32_t, int64_t, uint64_t

#define CH_LOGIC_INT_OP9(i, x) \
  CH_LOGIC_OP4_IMPL((template <unsigned M>), <<, const ch_int<M>&, x, const ch_int&) \
  CH_LOGIC_OP4_IMPL((template <unsigned M>), >>, const ch_int<M>&, x, const ch_int&)

#define CH_LOGIC_INT_FUNC8(i, x) \
  CH_LOGIC_FUNC4_IMPL((template <unsigned M, unsigned N>), ch_sll, const ch_int<M>&, x, const ch_int<N>&) \
  CH_LOGIC_FUNC4_IMPL((template <unsigned M, unsigned N>), ch_srl, const ch_int<M>&, x, const ch_int<N>&)
...
protected:
  ...
  CH_FOR_EACH(CH_LOGIC_INT_OP7, CH_SEP_SPACE, CH_LOGIC_INT_OP7_TYPES)  
  CH_FOR_EACH(CH_LOGIC_INT_OP9, CH_SEP_SPACE, CH_LOGIC_INT_OP8_TYPES)
};
...
CH_FOR_EACH(CH_LOGIC_INT_FUNC7, CH_SEP_SPACE, CH_LOGIC_INT_OP7_TYPES)
CH_FOR_EACH(CH_LOGIC_INT_FUNC8, CH_SEP_SPACE, CH_LOGIC_INT_OP8_TYPES)
