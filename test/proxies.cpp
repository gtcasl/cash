...
TEST_CASE("proxies", "[proxies]") {
  ...
  SECTION("slice", "[slice]") {
    ...
    TEST([]()->ch_bit1 {
      ch_bit2 c = ch_slice<2>(1100_b, 1) ^ 01_b;
      return (c == 11_b);
    });
    TEST([]()->ch_bit1 {
      ch_bit4 a(1100_b);
    ...
    TEST([]()->ch_bit1 {
      ch_bit2 c = ch_aslice<2>(1100_b, 1) ^ 01_b;
      return (c == 10_b);
    });
    TEST([]()->ch_bit1 {
      ch_bit4 a;
    ...
    TEST([]()->ch_bit1 {
      const ch_bit4 x(0101_b);
      return (x.aslice<2>(1) == x.slice<2>(2));
    });
  }
  ...
  SECTION("concat", "[concat]") {
    ...
    TEST([]()->ch_bit1 {
      ...
      auto b = (ch_slice<8>(k, 16), ch_slice<8>(k, 8));
      ch_tie(y, x) = a;
      ch_tie(w, z) = b;
