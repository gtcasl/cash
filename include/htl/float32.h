...
class ch_float32 : public ch_bit<32> {
public:
  using traits = logic_traits<32, true, ch_float32, ch_scbit<32>>;
  using base = ch_bit<32>;

  ch_float32(const logic_buffer_ptr& buffer = make_logic_buffer(32, CH_CUR_SLOC))
    : base(buffer)
  {}
  ...
  explicit ch_float32(float rhs, CH_SLOC) : base(*(uint32_t*)&rhs, sloc) {}
...
protected:
  ...
  CH_SCALAR_OP1_IMPL((), == const ch_float32&, float)
  CH_SCALAR_OP1_IMPL((), !=, const ch_float32&, float)
  CH_SCALAR_OP1_IMPL((), <, const ch_float32&, float)
  CH_SCALAR_OP1_IMPL((), <=, const ch_float32&, float)
  CH_SCALAR_OP1_IMPL((), >, const ch_float32&, float)
