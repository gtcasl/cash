...
public:  
  ...
  ch_reg_impl(CH_SRC_INFO)
    : base(make_logic_buffer(createRegNode(ch_width_v<T>, idname<T>(), srcinfo))) {
...
#define CH_REG_GEN(...) \
  ...
  ch_reg_impl(CH_FOR_EACH(CH_REG_GEN_DECL, , CH_SEP_COMMA, __VA_ARGS__), CH_SRC_INFO) \
    : base(make_logic_buffer(createRegNode( \
        get_lnode(T(CH_FOR_EACH(CH_REG_GEN_ARG, , CH_SEP_COMMA, __VA_ARGS__), srcinfo)), idname<T>(), srcinfo))) { \
    __next__ = std::make_unique<next_t>(getRegNextNode(get_lnode(*this))); \
  }
...
#undef CH_REG_GEN
  ...
  ch_reg_impl(const ch_reg_impl& other, CH_SRC_INFO)
    : base(make_logic_buffer(copyRegNode(get_lnode(other), idname<T>(), srcinfo))) {
    __next__ = std::make_unique<next_t>(getRegNextNode(get_lnode(*this)));
  }
...
auto ch_delay(const T& in, uint32_t delay = 1, CH_SRC_INFO) {
  ...
  return make_logic_type<R>(createRegNext(to_lnode<R>(in, srcinfo), delay, idname<R>(), srcinfo));
}
