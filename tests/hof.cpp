...
TEST_CASE("hof", "[hof]") {
  SECTION("map", "[map]") {
    TEST([]()->ch_bool {
      ...
      auto r = ch_map(a, [](const auto& x)->ch_bool { return ~x; });
      return (r.as_uint() == e);
    });
  ...
  SECTION("fold", "[fold]") {
    TEST([]()->ch_bool {
      ...
      auto r = ch_fold(a, [](const auto& x, const auto& y)->ch_bool { return x || y; }, 0);
      return (r == true);
    });
  ...
  SECTION("scan", "[scan]") {
    TEST([]()->ch_bool {
      ...
      auto r = ch_scan(a, [](const auto& x, const auto& y)->ch_bool { return x || y; }, 0);
  ...
  SECTION("zip", "[zip]") {
    TEST([]()->ch_bool {
      ...
      auto r = ch_zip(a, b, [](const auto& x, const auto& y)->ch_bool { return x || y; });
